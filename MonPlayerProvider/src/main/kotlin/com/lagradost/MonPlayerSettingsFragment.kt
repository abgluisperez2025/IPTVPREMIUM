...
class MonPlayerSettingsFragment(private val plugin: MonPlayerPlugin) : BottomSheetDialogFragment() {
    ...
    override fun onCreateView(
        ...
        addLinkButton.setOnClickListener(object : View.OnClickListener {
            override fun onClick(v: View?) {
                ...
                val radioGroup = credsView.findView<RadioGroup>("type_radio_group")
                ...
                if (!clipboardText.isNullOrEmpty() && clipboardText.startsWith("http")) {
                ...
                AlertDialog.Builder(context ?: throw Exception("Unable to build alert dialog"))
                    ...
                    .setPositiveButton("Save", object : DialogInterface.OnClickListener {
                        override fun onClick(p0: DialogInterface, p1: Int) {
                            ...
                            if (selectedId == -1 || name.isNullOrEmpty() || !link.startsWith("http")) {
                                showToast("Please fill in all information")
                            } else {
                                val selectedRadioButton = credsView.findView<RadioButton>(selectedId.toString())
                                val type = selectedRadioButton.text.toString()
                                val existingLinks = getKey<Array<Link>>("monplayer_links") ?: emptyArray()
                                if (existingLinks.any { it.name == name }) {
                                    showToast("This name already exists")
                                } else {
                                    CoroutineScope(Dispatchers.IO).launch {
                                        try {
                                            var data = Link(name, type, link)
                                            val response = app.get(link, mapOf("User-Agent" to "Dart/3.5 (dart:io)")).parsedSafe<ResponseHome>()?.search
                                            if (response != null) {
                                                val searchUrl = response.url + "?" + response.search_key + "="
                                                data = Link(name, type, link, searchUrl)
                                            }
                                            val updatedLinks = existingLinks.toMutableList().apply { add(data) }
                                            setKey("monplayer_links", updatedLinks)
                                            showToast("Save link sucess")
                                            plugin.reload()
                                        } catch (e: Exception) {
                                            e.printStackTrace()
                                            showToast("Error get link")
                                        }
                                    }     
                                }
                            }
                ...
                if (savedLinks.isEmpty()) {
                    ...
                    savedLinks.forEach { link ->
                        ...
                        deleteButton.setOnClickListener(object : View.OnClickListener {
                            override fun onClick(v: View?) {
                                try {
                                    val subSavedLinks = getKey<Array<Link>>("monplayer_links") ?: emptyArray()
                                    val updatedLinks = subSavedLinks.filter { it.name != link.name }.toTypedArray()
                                    setKey("monplayer_links", updatedLinks)
